#!/usr/bin/env bash
GRAALPATH="export PATH=/usr/local/bin/graalvm/bin:$PATH; export JAVA_HOME=/usr/local/bin/graalvm"
...
while [ true ]; do
    $GRAALPATH && java $RAM_FLAGS $GRAALVM_FLAGS $GC_FLAGS -jar ~/srv/server.jar nogui
